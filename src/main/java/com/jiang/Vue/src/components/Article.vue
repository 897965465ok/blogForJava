<template>
  <el-col
    :span="8"
    class="card-item"
    @click.native="readArticler(article.articlePath, article.id)"
  >
        ...
        <el-row class="footer">
          <div class="footer-top">
            ...
            <span
              ...
              @click.stop="like(article.id, article)"
            >{{ article.like }}</span>
          </div>
...
export default {
  ...
  name: "Articler",
  methods: {
    async readArticler(articlerPath, id) {
      await api.readArticler(id)
      this.$router.push({path: "markdown", query: {articlerPath, id}});
    },
    async like(id, article) {
      await api.like(id)
      article.like += 1;
    },
  },
